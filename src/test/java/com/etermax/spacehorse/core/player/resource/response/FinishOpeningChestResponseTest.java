...
public class FinishOpeningChestResponseTest {

	@Test
	public void whenCreateAFinishOpeningChestResponseThenTheJsonShouldBeCreatedProperly() throws Exception {
		// Given
		List<RewardResponse> rewards = newArrayList(aRewardResponseWith(10L), aRewardResponseWith(100L, "cardType"));
		// When
		FinishOpeningChestResponse response = new FinishOpeningChestResponse(rewards);
		// Then
		JsonNode actualJson = mapToJson(response);
		JsonNode expectedJson = readJson("com/etermax/spacehorse/player/resource/response/finish-opening-chest-response.json");
