...
public class QuestClaimResponseTest {

	@Test
	public void whenCreateAQuestApplyRewardResponseThenTheJsonShouldBeCreatedProperly() throws Exception {
		// Given
		List<RewardResponse> rewards = newArrayList(aRewardResponseWith(10L), aRewardResponseWith(100L, "cardType"));
		int refreshTimeInSeconds = 100;
		// When
		QuestClaimResponse response = new QuestClaimResponse(rewards, refreshTimeInSeconds);
		// Then
		JsonNode actualJson = mapToJson(response);
		JsonNode expectedJson = readJson("com/etermax/spacehorse/quest/resource/response/claim-quest-response.json");
